  ...
  public init(): void {
    ...
    editor.ui.componentFactory.add(`menuBar:${FONT_FAMILY}`, (locale) => {
      ...
      for (const definition of listOptions) {
        ...
        buttonView
          .bind(
            ...(Object.keys(definition.model) as Array<
              keyof MenuBarMenuListItemButtonView
            >)
          )
          .to(definition.model);
 ...
 */
function _prepareListOptions(
  options: Array<FontFamilyOption>,
  command: FontFamilyCommand
  ...
  for (const option of options) {
    ...
    def.model.bind("isOn").to(command, "value", (value) => {
      ...
      return (
        value.split(",")[0].replace(/'/g, "").toLowerCase() ===
        option.model.toLowerCase()
      );
    });
    ...
    if (option.view && typeof option.view !== "string" && option.view.styles) {
      def.model.set(
        "labelStyle",
        `font-family: ${option.view.styles["font-family"]}`
      );
    }
